...
from traffic_generator.web.catalog import (
    CatalogPayload,
    build_catalog,
...
def _unblock_link(meta: object | None) -> str | None:
    if not isinstance(meta, dict):
        return None

    meta_object = cast("dict[str, object]", meta)
    link = meta_object.get("unblock_link")
...
def dogfooding_catalog_page(request: Request) -> HTMLResponse:
    ...
    template_context: dict[str, Any] = {
        "request": request,
        "global_scenarios": catalog.global_scenarios,
    ...
    step_results: list[ScenarioStepRunPayload] = [
        {
            ...
            "unblock_link": _unblock_link(step_result.meta),
        }
